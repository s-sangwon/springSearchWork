...
/*
 * jQuery(document.ready(){ 코드 작성 });
 * => 이 페이지가 브라우저에 읽어들이기(로딩)가 완료되면
 {} 안의 내용을 자동 실행시켜라.는 의미
 => jQuery 는 줄여서 $로 표기해도 됨
 => 위의 구문은 줄여서 $(function() { 이벤트 코드 작성 }); 표기가능
 => 자바스크립트로는 window.onload = function() { 이벤트 코드 작성 }
 */
 $(function() {
     // 이 안에서 작성된 코드는 이벤트가 발생될 때까지 대기상태가 됨. 
     //jQuery('태그선택자').실행할메소드(전달값, ..........);
     // 인포트의 네임 속성을 실행할려면 on(); 하면 된다.
     $("input[name=item]").on("change", function() {
        //change event가 발생한 radio 와 연결된 폼만 보여지게 하고,
        //나머지 폼들은 안보이게 처리함. 
...
// 			//ajax 는 리스트가 누적이되서 계속 쌓이다보면 실행속도가 느려질수 있음
// 			//ajax를 쓸지 창을다시열지 판단해야함
// 			location.href = "${ pageContext.servletContext.contextPath }/loginok.do?userid=" + userid + "&login_ok=" + element.value;
// 		}else {
// 			console.log("로그인 제한 헤제함");
// 			location.href = "${ pageContext.servletContext.contextPath }/loginok.do?userid=" + userid + "&login_ok=" + element.value;
// 		}
		
		if (element.checked == true) {
			console.log("로그인 제한 또는 해제함");
			location.href = "${ pageContext.servletContext.contextPath }/loginok.do?userid=" + userid + "&login_ok=" + element.value;
		}
...
<table align="center" border="1" cellspacing="0" cellpadding="3">
	...
	<c:forEach items="${ requestScope.list }" var="m" varStatus="state">
	<tr>
		<td>${ m.userid }</td>
		<td>${ m.username }</td>
		...
		<td><fmt:formatDate value="${ m.lastmodified }" type="both" dateStyle="medium"/></td>
		<td>
